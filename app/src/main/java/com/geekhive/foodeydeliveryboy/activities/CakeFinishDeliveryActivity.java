...
import com.geekhive.foodeydeliveryboy.beans.cakeorderHistory.Cake;
...
public class CakeFinishDeliveryActivity extends AppCompatActivity implements OnResponseListner {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        vB_cash_collected.setOnSlideCompleteListener(new SlideToActView.OnSlideCompleteListener() {
            ...
            public void onSlideComplete(SlideToActView slideToActView) {
                if (orderListOut.getOrderHistory().get(position_).getCakeCart().getPaymentStatus().equals("1")){
                    CallAlreadyService();
                } else {
                    CallDeliverService();
                }
    ...
    private void CallCashService() {
        if (this.mDetector.isConnectingToInternet()) {
            new WebServices(this).CashCakePay(WebServices.Foodey_Services,
                    WebServices.ApiType.cpay, orderListOut.getOrderHistory().get(position_).getCakeCart().getId(), Prefs.getUserId(this));
        }
    ...
    private void CallOnlineService() {
        if (this.mDetector.isConnectingToInternet()) {
            new WebServices(this).OnlineCakePay(WebServices.Foodey_Services,
                    WebServices.ApiType.opay, orderListOut.getOrderHistory().get(position_).getCakeCart().getId(), Prefs.getUserId(this));
        }
    ...
    private void CallAlreadyService() {
        if (this.mDetector.isConnectingToInternet()) {
            new WebServices(this).AlreadyCakePay(WebServices.Foodey_Services,
                    WebServices.ApiType.apay, orderListOut.getOrderHistory().get(position_).getCakeCart().getId(), Prefs.getUserId(this));
        }
    ...
    private void CallDeliverService() {
        if (this.mDetector.isConnectingToInternet()) {
            new WebServices(this).DeliveredCakeOrder(WebServices.Foodey_Services,
                    WebServices.ApiType.orderDelivered, Prefs.getUserId(this), orderListOut.getOrderHistory().get(position_).getCakeCart().getId());
        }
        ...
        if (apiType == WebServices.ApiType.restaurantList) {
            if (!isSucces) {
                ...
                if (!isSucces || orderListOut == null) {
                    ...
                    latitude = Double.parseDouble(orderListOut.getOrderHistory().get(position_).getCakeStore().getLatitude());
                    longitude = Double.parseDouble(orderListOut.getOrderHistory().get(position_).getCakeStore().getLongitude());
                    ...
                    String confTest ="Give " + orderListOut.getOrderHistory().get(position_).getCakeCartDetail().size() + " item(s) to customer";
                    ...
                    if (orderListOut.getOrderHistory().get(position_).getCakeCart().getPaymentStatus().equals("0")){
                        String cash = " Collect from customer ₹ " + orderListOut.getOrderHistory().get(position_).getCakeCart().getGrandTotal() +" ";
                        vT_cash_amount.setText(cash);
                        String csh = "Collect ₹ " + orderListOut.getOrderHistory().get(position_).getCakeCart().getGrandTotal() + " cash";
                        vB_total_cash_amount.setText(csh);
                        vB_total_cash_amount.setVisibility(View.VISIBLE);
                    } else {
                        String cash = " Online Payment of ₹ " + orderListOut.getOrderHistory().get(position_).getCakeCart().getGrandTotal() +" ";
                        vT_cash_amount.setText(cash);
                        vB_total_cash_amount.setVisibility(View.GONE);
                        vB_finish_do.setVisibility(View.GONE);
                        vB_cash_collected.setVisibility(View.VISIBLE);
            ...
            if (!isSucces || orderDelivered == null) {
                ...
                Intent intent = new Intent(CakeFinishDeliveryActivity.this, OrderCompletedActivity.class);
                intent.putExtra("trip_earn", orderListOut.getOrderHistory().get(position_).getCakeCart().getDeliveryBoyCharge());
    ...
    private void InitializeRatepopup() {
        RatePopup = new Dialog(CakeFinishDeliveryActivity.this);
...
//                vR_smiley.setVisibility(View.GONE);
    ...
    private void InitializeOnlinePopup() {
        paypopup = new Dialog(CakeFinishDeliveryActivity.this);
