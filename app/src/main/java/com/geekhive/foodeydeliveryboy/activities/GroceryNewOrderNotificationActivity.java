...
public class GroceryNewOrderNotificationActivity extends AppCompatActivity implements OnResponseListner {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        slide_to_accept.setOnSlideCompleteListener(new SlideToActView.OnSlideCompleteListener() {
            ...
            public void onSlideComplete(SlideToActView slideToActView) {
                CallOrderService(Prefs.getUserId(GroceryNewOrderNotificationActivity.this), orderListOut.getOrder().getGrocery().get(position_).getGCart().getId());
            }
        });
    ...
    private void CallOrderService(String del_id, String cart_id) {
        if (this.mDetector.isConnectingToInternet()) {
            new WebServices(this).ConfirmGrocOrder(WebServices.Foodey_Services,
        ...
        if (apiType == WebServices.ApiType.newOrderList) {
            if (!isSucces) {
                ...
                if (!isSucces || orderListOut == null) {
                    ...
                    String str = orderListOut.getOrder().getGrocery().get(position_).getStore().getName() + "\n" + orderListOut.getOrder().getGrocery().get(position_).getStore().getAddress();
            ...
            if (!isSucces || orderConfirmation == null) {
                ...
                Intent intent=new Intent(GroceryNewOrderNotificationActivity.this, GroceryRestaurantReachActivity.class);
                intent.putExtra("position", position_);
                intent.putExtra("lat", orderListOut.getOrder().getGrocery().get(position_).getStore().getLatitude());
                intent.putExtra("lang", orderListOut.getOrder().getGrocery().get(position_).getStore().getLongitude());
                startActivity(intent);
                overridePendingTransition( R.anim.left_in, R.anim.left_out);
                GroceryNewOrderNotificationActivity.this.finish();
            }
        }
