...
public class GroceryGetOTPActivity extends AppCompatActivity implements OnResponseListner {
    Button get_otp;
    Dialog OtpPopup;
    ...
    private void CallResendService() {
        if (this.mDetector.isConnectingToInternet()) {
            new WebServices(this).ResendDelOtp(WebServices.Foodey_Services,
                    WebServices.ApiType.resendOtp, orderListOut.getOrderHistory().get(position_).getGCart().getOrderId(), Prefs.getUserId(this));
        }
                ...
                if (!isSucces || verifyOtp == null) {
                    ...
                    if (verifyOtp.getMessage() != null) {
                        if (!verifyOtp.getMessage().isEmpty()) {
                            ...
                            startService(intent);*/
                            Intent intenti = new Intent(GroceryGetOTPActivity.this, PickOrderActivity.class);
                            intenti.putExtra("position", position_);
                            intenti.putExtra("lat", orderListOut.getOrderHistory().get(position_).getStore().getLongitude());
    ...
    private void InitializeConfirmOtppopup() {
        OtpPopup = new Dialog(GroceryGetOTPActivity.this);
