...
public class FinishDeliveryActivity extends AppCompatActivity implements OnResponseListner {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        vB_cash_collected.setOnSlideCompleteListener(new SlideToActView.OnSlideCompleteListener() {
            ...
            public void onSlideComplete(SlideToActView slideToActView) {
                if (orderListOut.getOrderHistory().get(position_).getCart().getPaymentStatus().equals("1")){
                    CallAlreadyService();
                } else {
                    CallDeliverService();
                }
    ...
    private void CallCashService() {
        if (this.mDetector.isConnectingToInternet()) {
            new WebServices(this).CashPay(WebServices.Foodey_Services,
                    WebServices.ApiType.cpay, orderListOut.getOrderHistory().get(position_).getCart().getId(), Prefs.getUserId(this));
        }
    ...
    private void CallOnlineService() {
        if (this.mDetector.isConnectingToInternet()) {
            new WebServices(this).OnlinePay(WebServices.Foodey_Services,
                    WebServices.ApiType.opay, orderListOut.getOrderHistory().get(position_).getCart().getId(), Prefs.getUserId(this));
        }
    ...
    private void CallAlreadyService() {
        if (this.mDetector.isConnectingToInternet()) {
            new WebServices(this).AlreadyPay(WebServices.Foodey_Services,
                    WebServices.ApiType.apay, orderListOut.getOrderHistory().get(position_).getCart().getId(), Prefs.getUserId(this));
        }
    ...
    private void CallDeliverService() {
        if (this.mDetector.isConnectingToInternet()) {
            new WebServices(this).DeliveredOrder(WebServices.Foodey_Services,
                    WebServices.ApiType.orderDelivered, Prefs.getUserId(this), orderListOut.getOrderHistory().get(position_).getCart().getId());
        }
        ...
        if (apiType == WebServices.ApiType.restaurantList) {
            if (!isSucces) {
                ...
                if (!isSucces || orderListOut == null) {
                    ...
                    String confTest ="Give " + orderListOut.getOrderHistory().get(position_).getCartDetail().size() + " item(s) to customer";
                    ...
                    if (orderListOut.getOrderHistory().get(position_).getCart().getPaymentStatus().equals("0")){
                        String cash = " Collect from customer ₹ " + orderListOut.getOrderHistory().get(position_).getCart().getGrandTotal() +" ";
                        vT_cash_amount.setText(cash);
                        String cashT = " Collected from Customer : ₹ " + orderListOut.getOrderHistory().get(position_).getCart().getGrandTotal() +" ";
                        vT_cash_visible.setText(cashT);
                        String csh = "Collect ₹ " + orderListOut.getOrderHistory().get(position_).getCart().getGrandTotal() + " cash";
                        vB_total_cash_amount.setText(csh);
                        vB_total_cash_amount.setVisibility(View.VISIBLE);
                    } else {
                        String cash = " Online Payment of ₹ " + orderListOut.getOrderHistory().get(position_).getCart().getGrandTotal() +" ";
                        vT_cash_amount.setText(cash);
                        String cashT = " Collected from Customer : ₹ " + orderListOut.getOrderHistory().get(position_).getCart().getGrandTotal() +" ";
                        vT_cash_visible.setText(cashT);
                        vB_total_cash_amount.setVisibility(View.GONE);
            ...
            if (!isSucces || orderDelivered == null) {
                ...
                Intent intent = new Intent(FinishDeliveryActivity.this, OrderCompletedActivity.class);
                intent.putExtra("trip_earn", orderListOut.getOrderHistory().get(position_).getCart().getDeliveryBoyCharge());
    ...
    private void InitializeRatepopup() {
        RatePopup = new Dialog(FinishDeliveryActivity.this);
...
//                vR_smiley.setVisibility(View.GONE);
    ...
    private void InitializeOnlinePopup() {
        paypopup = new Dialog(FinishDeliveryActivity.this);
