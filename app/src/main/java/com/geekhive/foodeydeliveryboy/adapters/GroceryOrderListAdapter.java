...
public class GroceryOrderListAdapter extends RecyclerView.Adapter<GroceryOrderListAdapter.MyViewHolder> {
    public Typeface Montserrat_Regular;
    public Typeface Montserrat_SemiBold;
    Context context;
    GrocOrderHistory orderHistory;
    //ProductCategoryList productCategoryList;

    public GroceryOrderListAdapter(Context context, GrocOrderHistory orderHistory) {
    ...
    public void onBindViewHolder(MyViewHolder holder, final int position) {
        ...
        holder.orderId.setText(orderHistory.getOrderHistory().get(position).getGCart().getOrderId());
        holder.orderTime.setText(orderHistory.getOrderHistory().get(position).getGCart().getDate());
        if (orderHistory.getOrderHistory().get(position).getGCart().getStatus().equals("3")) {
            holder.viewOrder.setText("On the way");
        } else if (orderHistory.getOrderHistory().get(position).getGCart().getStatus().equals("2")){
            holder.viewOrder.setText("Canceled");
        } else if (orderHistory.getOrderHistory().get(position).getGCart().getStatus().equals("4")){
